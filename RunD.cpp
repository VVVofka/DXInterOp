...
void RunD::RunK(const array<DrQuadro, 2>& srcd,
			   array<DrQuadro, 2>& dstd,
			   const array<int, 2>& dsta,
			   const array<FLT2, 1>& masksD,
			   double kLay){
	parallel_for_each(srcd.extent, [&srcd, &dstd, &dsta, &masksD, kLay](index<2> idx) restrict(amp){
		const INT2 src(idx);
		const INT2 dst = src * 2;
		...
		const FLT2* srcitems = srcd[src.y][src.x].items;
		FLT2* dstitems = dstd[y0][x0].items;
		dstitems->set(srcitems->y + masksD[nmask].y * kLay, srcitems->x + masksD[nmask].x * kLay); nmask++;
		(++dstitems)->set(srcitems->y + masksD[nmask].y * kLay, srcitems->x + masksD[nmask].x * kLay); nmask++;
		(++dstitems)->set(srcitems->y + masksD[nmask].y * kLay, srcitems->x + masksD[nmask].x * kLay); nmask++;
		(++dstitems)->set(srcitems->y + masksD[nmask].y * kLay, srcitems->x + masksD[nmask].x * kLay); nmask++;
		...
		(++dstitems)->set(srcitems->y + masksD[nmask].y * kLay, srcitems->x + masksD[nmask].x * kLay); nmask++;
		(++dstitems)->set(srcitems->y + masksD[nmask].y * kLay, srcitems->x + masksD[nmask].x * kLay); nmask++;
		(++dstitems)->set(srcitems->y + masksD[nmask].y * kLay, srcitems->x + masksD[nmask].x * kLay);
	});
} // ///////////////////////////////////////////////////////////////////////////////////////////////
